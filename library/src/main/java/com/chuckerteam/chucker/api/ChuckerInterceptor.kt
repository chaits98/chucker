...
public class ChuckerInterceptor internal constructor(
    ...
    init {
        PrefUtils.getInstance(context).setRedactedHeaders(headersToRedact)
    }

    /** Adds [headerName] into [headersToRedact] */
    public fun redactHeader(vararg headerName: String) {
        val previousHeaders = ArrayList<String>()
        previousHeaders.addAll(PrefUtils.getInstance(context).getRedactedHeaders())
        previousHeaders.addAll(headerName)
        PrefUtils.getInstance(context).setRedactedHeaders(previousHeaders.toSet())
    }
